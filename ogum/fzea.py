...
# ==============================================================================
    ...
    try:
        ...
        if missing_cols:
            ...
            raise ValueError(
                f"Não foi possível encontrar as seguintes colunas essenciais: {', '.join(missing_cols)}. Verifique o cabeçalho do arquivo TXT ou o log de depuração."
            )
        ...
        df_flash["Retracao_Ajuste4"] = (
            0
            if delta_retracao_medida == 0
            else ((df_flash["Retracao_Corrigida"] - retracao_no_inicio_flash) / delta_retracao_medida)
            * retracao_global_mm
        )
...
uploader = widgets.FileUpload(
    accept=".txt",
    description="1. Carregar Arquivo TXT",
...
def on_process_button_clicked(b):
    ...
    with status_output:
        ...
        if not all([diametro, esp_inicial, esp_final]):
            print("Erro: Todos os campos de dimensão devem ser preenchidos.")
            return
...
def on_basic_graphs_clicked(b):
    ...
    with graphs_output:
        ...
        if processed_df is None:
            print("Por favor, processe os dados primeiro.")
            return
        fig1, axs1 = plt.subplots(2, 2, figsize=(12, 8))
        fig1.suptitle("Análise de Flash Sintering - Painel Principal", fontsize=16)
        t = processed_df["Tempo_Normalizado"]
        xlabel = "Tempo Normalizado (s)"
        axs1[0, 0].plot(t, processed_df["Densidade_Potencia_W_cm3"], "r")
        axs1[0, 0].set(title="Densidade de Potência vs. Tempo", xlabel=xlabel, ylabel="W/cm³")
        axs1[0, 0].grid(True)
        axs1[0, 1].plot(t, processed_df["Temp_Amostra"], "g")
        axs1[0, 1].set(title="Temperatura vs. Tempo", xlabel=xlabel, ylabel="°C")
        axs1[0, 1].grid(True)
        axs1[1, 0].plot(t, processed_df["Densidade_Corrente_A_cm2"], "b")
        axs1[1, 0].set(title="Densidade de Corrente vs. Tempo", xlabel=xlabel, ylabel="A/cm²")
        axs1[1, 0].grid(True)
        axs1[1, 1].plot(t, processed_df["Retracao_Ajuste4"], "m")
        axs1[1, 1].set(title="Retração Ajustada vs. Tempo", xlabel=xlabel, ylabel="Retração (mm)")
        axs1[1, 1].grid(True)
...
def on_adv_graphs_clicked(b):
    ...
    with graphs_output:
        ...
        fig2.suptitle("Análise de Flash Sintering - Painel Adicional", fontsize=16)
        t = processed_df["Tempo_Normalizado"]
        xlabel = "Tempo Normalizado (s)"
        axs2[0, 0].plot(t, processed_df["Taxa_Retracao_vs_Tempo"], "c")
        axs2[0, 0].set(title="Taxa de Retração vs. Tempo", xlabel=xlabel, ylabel="mm/s")
        axs2[0, 0].grid(True)
        axs2[0, 1].plot(processed_df["Temp_Amostra"], processed_df["Taxa_Retracao_vs_Temp"], "y")
        axs2[0, 1].set(title="Taxa de Retração vs. Temperatura", xlabel="Temperatura (°C)", ylabel="mm/°C")
        axs2[0, 1].grid(True)
        axs2[1, 0].plot(t, processed_df["Temp_BBR_C"], "orange")
        axs2[1, 0].set(title="Temperatura BBR vs. Tempo", xlabel=xlabel, ylabel="°C")
        axs2[1, 0].grid(True)
        axs2[1, 1].plot(processed_df["Temp_BBR_C"], processed_df["Retracao_Ajuste4"], "purple")
        axs2[1, 1].set(title="Retração vs. Temperatura BBR", xlabel="BBR (°C)", ylabel="Retração (mm)")
        axs2[1, 1].grid(True)
        plt.tight_layout(rect=[0, 0.03, 1, 0.95])
        plt.show()
